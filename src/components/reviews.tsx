...
export const RatingAverage = ({ reviews }: { reviews: Review[] }) => {
	if (!reviews.length) return <p className="italic whitespace-nowrap">No reviews yet</p>;
	const avgRating = reviews.map((r) => r.rating).reduce((a, b) => a + b) / reviews.length;
	return (
...
export const ReviewList = ({ reviews }: { reviews: Review[] }) => {
	return reviews.map((r, i) => (
		<div className="w-full border-b-2 px-2 py-4 last-of-type:border-b-0" key={i}>
			<div className="flex justify-between">
				<Stars rating={r.rating} />
				<span className="text-sm text-gray-dark">{new Date(r.date).toDateString()}</span>
			</div>
